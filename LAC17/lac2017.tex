...
\begin{abstract}
\begin{contentsmall}
This paper introduces a use case for a library written in the Faust language that provides tools to synthesize sound using physical modeling. The first section gives an overview of Faust and physical modeling theory. In the second section, we focus on the writing of the library in Faust and on what can be synthesized with those tools. The last section describes two python scripts aiming to synthesize modeled objects resonance.
\end{contentsmall}
\end{abstract}
...
Sound synthesis is an active subject of research and many programming languages have been designed and used for that purpose. Physical modeling synthesis gained quite a lot success because it can provide tools to recreate the sound of real instruments. This synthesis technique has evolved during the last twenty years of the $20^{th}$ century and was developed with the programming languages of that period. T

The more recent Faust audio language allows the programer to quickly build sound processors with a nice block diagram view. Our interest here is to build a modular library that supplies small pieces of instruments (a bore, a string, a bridge, a bow, a hammer ...) to be used to recreate sound when put together. It would even be possible to create new instruments with new assemblies.
